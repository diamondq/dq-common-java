...
public class DataService {
	...
	public DataService(Toolkit pToolkit, Config pConfig) {
		...
		sd = sd.addPropertyDefinition(
			mToolkit.createNewPropertyDefinition(mScope, "registrationLocation", PropertyType.String));
		sd = sd.addPropertyDefinition(
			mToolkit.createNewPropertyDefinition(mScope, "authorizationLocation", PropertyType.String));
		sd = sd.addPropertyDefinition(
			mToolkit.createNewPropertyDefinition(mScope, "certificateLocation", PropertyType.String));
		...
		sd = sd
			.addPropertyDefinition(mToolkit.createNewPropertyDefinition(mScope, "domainKeyPair", PropertyType.String));
	...
	public @Nullable ChallengeState lookupChallengeState(String pToken) {
		Structure structure = mToolkit
			.createStructureRefFromParts(mScope, null, null, mChallengeStateDef, Collections.singletonList(pToken))
			.resolve();
		if (structure == null)
			return null;
	...
	public @Nullable PersistedState lookupPersistedState(String pDomain) {
		Structure structure = mToolkit
			.createStructureRefFromParts(mScope, null, null, mPersistedStateDef, Collections.singletonList(pDomain))
			.resolve();
		if (structure == null)
			return null;
