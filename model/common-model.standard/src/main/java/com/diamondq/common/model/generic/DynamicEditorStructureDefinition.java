...
public class DynamicEditorStructureDefinition implements EditorStructureDefinition {
  ...
  public DynamicEditorStructureDefinition(Scope pScope, StructureDefinition pStructureDefinition) {
    ...
    for (PropertyDefinition pd : mStructureDef.getAllProperties().values()) {
      ...
      if (label != null) displayName = label.resolve(null);
      if (displayName == null) displayName = pd.getName();
      if (pd.getKeywords().containsEntry(CommonKeywordKeys.EDITOR_VISIBLE, CommonKeywordValues.FALSE) == true) continue;
      if (pd.isPrimaryKey() == true) primaryPDs.put(displayName, pd);
      else otherPDs.put(displayName, pd);
  ...
  private static @Nullable EditorPropertyDefinition buildEditorPropertyDescription(Scope pScope, PropertyDefinition pPD,
    ...
    switch (pPD.getType()) {
      ...
      case String: {
        ...
        if (pPD.isPrimaryKey() == true) epd = epd.setMandatory(true);
        return epd;
      }
      case StructureRef: {
        return null;
      ...
      case StructureRefList: {
        if (pPD.getKeywords().containsEntry("ATStructureChooser", "child") == true) return null;
        EditorPropertyDefinition epd = pScope.getToolkit().createNewEditorPropertyDefinition(pScope);
        epd = epd.setLabel(pPD.getLabel()).setName(pPD.getName());
