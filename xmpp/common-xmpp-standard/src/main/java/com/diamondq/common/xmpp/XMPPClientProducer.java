...
public class XMPPClientProducer {
	...
	public @Nullable @Default @XMPPClientRef("") XmppClient getClient(InjectionPoint pInjectionPoint, Config pConfig,
		...
		for (XMPPServerInfo serverInfo : pServerInfos) {
			...
			TcpConnectionConfiguration.Builder tcpBuilder = TcpConnectionConfiguration.builder();

			tcpBuilder = tcpBuilder.hostname(serverInfo.hostname);

			tcpBuilder = tcpBuilder.port(serverInfo.port);

			Integer connectTimeout = pConfig.bind("xmpp.tcp.connect-timeout", Integer.class);
			if (connectTimeout != null)
				tcpBuilder = tcpBuilder.connectTimeout(connectTimeout);

			Integer keepAliveInterval = pConfig.bind("xmpp.tcp.keep-alive-interval", Integer.class);
			if (keepAliveInterval != null)
				tcpBuilder = tcpBuilder.keepAliveInterval(keepAliveInterval);

			Boolean secure = pConfig.bind("xmpp.tcp.secure", Boolean.class);
			if (secure != null)
				tcpBuilder = tcpBuilder.secure(secure);
			...
			if (proxyHost != null) {
				...
				tcpBuilder = tcpBuilder.proxy(proxy);
			}

			TcpConnectionConfiguration tcpConfiguration = tcpBuilder.build();
			configurationList.add(tcpConfiguration);
			...
			if (debuggerName != null) {
				try {
					...
					Class<? extends XmppDebugger> debuggerClass =
						(Class<? extends XmppDebugger>) Class.forName(debuggerName);
			...
			if (reconnectType != null) {
				...
				if ("backoff".equals(reconnectType)) {
					...
					reconnectionStrategy =
						ReconnectionStrategy.truncatedBinaryExponentialBackoffStrategy(slotTime, ceilingTime);
				}
