...
@ApplicationScoped
public class XMPPServerLookup {

	private static final Logger	sLogger	= LoggerFactory.getLogger(XMPPServerLookup.class);
	...
	public SortedSet<XMPPServerInfo> getXMPPServerList(@Nullable String pDNSDomain) {
			...
			case Lookup.SUCCESSFUL: {
				for (Record record : records) {
					if (record instanceof SRVRecord) {
						...
						default:
							sLogger.error("Unable to lookup TXT record for {}. Error={}", serviceStr,
								txtLookup.getErrorString());
						}
						builder.add(new XMPPServerInfo(pDNSDomain, srvRecord.getTarget().toString(),
							srvRecord.getPort(), srvRecord.getPriority(), srvRecord.getWeight(), infoBuilder.build()));
					}
				}
				break;
			}
		...
		if (configHostName != null) {
			...
			builder.add(new XMPPServerInfo(domain, configHostName, configPort, Integer.MAX_VALUE, 0,
