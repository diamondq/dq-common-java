...
public class SetupNode {
  ...
  private NodeInfo getNodeInfo(ComputeService pComputeService, String pNodeId) {
    ...
    statementBuilder.add(
      Statements.literal("	[[ $(tar cf - /var/chef/cookbooks/DiamondNode 2>/dev/null | md5sum -b) =~ $md5pattern ]]"));
    statementBuilder.add(Statements.literal(""));
    statementBuilder.add(Statements.literal("	if [ ${BASH_REMATCH[1]}	= '7899a07b7bc0abc2621d92092705ef00' ]; then"));
