...
public class TestConstructionInfoBuilder {
	...
	public void dependentConstructor() {
		ConstructorInfo info = ConstructorInfoBuilder.builder().constructorClass(TestClassWithObjConstructor.class)
			.cArg().type(TestDep.class).value(null).optional().build().build();
	...
	public void failIfRequiredParamIsNull() {
		try {
			ConstructorInfo info =
				ConstructorInfoBuilder.builder().constructorClass(TestClassWithEmptyConstructor.class).cArg()
					.type(TestDep.class).value(null).build().build();
			Assert.assertNotNull(info);
		}
		catch (ExtendedIllegalArgumentException ex) {
