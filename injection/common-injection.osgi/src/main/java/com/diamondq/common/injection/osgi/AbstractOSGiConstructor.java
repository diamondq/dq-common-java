    ...
    synchronized (this) {
      ...
      for (int i = 0; i < mInfo.filters.length; i++) {
        ...
        try {
          ...
          throw new IllegalArgumentException("Unable to parse the filter " + finalFilterStr + " within " + errorId, ex);
        }
      ...
      for (int i = 0; i < mInfo.constructionArgs.length; i++) {
        ...
        if (arg.propertyFilterKey != null) {
          ...
          if (tracker == null) {
            ...
            if (references.isEmpty()) {
              if (arg.required == Boolean.TRUE) {
                sLogger.trace("\tUnable to find references to arg #{}: propertyFilterKey={}", i, arg.propertyFilterKey);
                available = false;
                break;
              }
            }
          ...
          if (value == null) {
            ...
            if (value == null) {
              if (arg.required == Boolean.TRUE) {
                sLogger.trace("\tUnable to resolve reference to arg #{}: propertyValueKey={}", i, arg.propertyValueKey);
                available = false;
                break;
              }
            }
